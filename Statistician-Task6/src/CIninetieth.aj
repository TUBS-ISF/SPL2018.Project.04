...
public privileged aspect CIninetieth {
	...
	after(List<Double> list, Statistics s): execution(double Statistics.execute(List<Double>)) && args(list) && this(s) {
		if(!(list == null || list.isEmpty())) {
			...
			double result = 1.65 * (standardDeviation/(double) Math.sqrt(list.size()));
			
			System.out.printf(s.counter + ". 90th Confidence Interval: %.5f \u00B1 %.5f \n",  average ,result);
			s.counter++;
		}	
	}
}
